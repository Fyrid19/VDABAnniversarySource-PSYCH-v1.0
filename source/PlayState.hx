...
class PlayState extends MusicBeatState
{
	...
	var floaters:Array<String> = ['dave-3d', 'madman-dave', 'bambi-3d', 'expunged', 'bamber-rush', 'expunged-looky', '3d-bf', 'bandu', 'lenzo', 'barren', 'origin-lenzo', 'origin-barren', 'expunged-redsign', 'bandu-redesign'];
	var sideFloaters:Array<String> = ['bamber-rush', 'lenzo', 'origin-lenzo', 'barren', 'origin-barren', 'expunged-looky'];

	var sineElap:Int = 0;

	var earthquakers:Array<String> = ['expunged', 'expunged-looky', 'bambi-3d'];
	var dudesWhoTakeUrHealthBecauseTheyCan:Array<String> = ['expunged', 'expunged-looky', 'bambi-3d'];
	...
	public var cameraSpeed:Float = 2.3;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'redsky': //polygonized and furiosity
				defaultCamZoom = 0.85;
				curStage = 'redsky';
			...
			case 'farmDay':		
					...
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('backgrounds/skies/sky'));
					bg.antialiasing = true;
				...
				case 'farmSunset':
					...
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('backgrounds/skies/sky_sunset'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.2, 0.2);
					bg.active = true;
				...
				case 'farmNight':
					...
					var bg:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('backgrounds/skies/sky_night'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.2, 0.2);
					bg.active = true;
					...
					redbg = new FlxSprite(-600, -200).loadGraphic(Paths.image('backgrounds/redsky'));
					redbg.antialiasing = true;
					redbg.scrollFactor.set(0.6, 0.6);
					redbg.active = false;
					redbg.visible = false;
					...
					testshader.waveFrequency = 5;
					testshader.waveSpeed = 2;
					redbg.shader = testshader.shader;

					greenbg = new FlxSprite(-600, -200).loadGraphic(Paths.image('backgrounds/cheater'));
					greenbg.antialiasing = true;
					greenbg.scrollFactor.set(0.6, 0.6);
					...
					greenbg.shader = testshader2.shader;
					curbg = greenbg;
		...
		/*switch(charSelection) {
			...
			case 1:
				boyfriend = new Boyfriend(0, 0, 'dave');
			case 2:
				boyfriend = new Boyfriend(0, 0, 'bambi');
			default:
		...
		watermark = new FlxText(scoreTxt.x + 30, scoreTxt.y + 30, 0, "Barren Engine v" + MainMenuState.barrenEngineVer + " | " + curSong + " | " + storyDifficultyText.toUpperCase(), 20);
		...
		credits = new FlxText(watermark.x, watermark.y + 30, 0, "", 20);
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "hi yt | BOTPLAY", 32);
	...
	function songCardPopup(durationIn:Float = 1, durationOut:Float = 1){
		var songName:String = Paths.formatToSongPath(SONG.song);
		var leBox:FlxSprite = new FlxSprite(-1500, 300).loadGraphic(Paths.image('other/songCard'));
		leBox.scrollFactor.set();
		leBox.cameras = [camHUD];
		leBox.color = FreeplayState.intendedColor;
		add(leBox);
		var leSongName:FlxText = new FlxText(-1200, 320, 0, curSong, 30);
		leSongName.scrollFactor.set();
		leSongName.cameras = [camHUD];
		leSongName.setFormat(Paths.font("comic.ttf"), 50, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(leSongName);
		new FlxTimer().start(1, function(tmr:FlxTimer)
			{
		FlxTween.tween(leBox, {x: 0}, durationIn, {type:PERSIST, ease:FlxEase.backInOut});
		FlxTween.tween(leSongName, {x: 130}, durationIn, {type:PERSIST, ease:FlxEase.backInOut});
			});
		new FlxTimer().start(6, function(tmr:FlxTimer)
			{
		FlxTween.tween(leSongName, {x: -500}, durationOut, {type:PERSIST, ease:FlxEase.backInOut});
		FlxTween.tween(leBox, {x: -700}, durationOut, {type:PERSIST, ease:FlxEase.backInOut});
			});
		}
	...
	override public function update(elapsed:Float)
	{
		...
		switch (Paths.formatToSongPath(SONG.song)) {
			case 'triple-trouble':
				switch (curStep) { // I CANT GET BACKGROUND SWITCHES TO WORK
					case 0: //bandu
						FlxG.camera.flash(0xFF00FF15 , 3);
					case 1040 | 2320 | 4112: // expunged. enough said
						FlxG.camera.flash(0xFFFF0000, 1);
					case 1296: // a wild bambi appears
						FlxG.camera.flash(0xFF34AC3E, 1);
					case 2832: // you arent cheating but screw you anyways
						FlxG.camera.flash(0xFF63DD6E , 1);
				}
		}
		...
		if(floaters.contains(dad.curCharacter.toLowerCase()))
		{
			dad.y += (Math.sin(elapsedtime) * 0.6);
		}
		if(sideFloaters.contains(dad.curCharacter.toLowerCase()))
		{
			dad.x += (Math.cos(elapsedtime) * 0.6);
		}
		if(floaters.contains(boyfriend.curCharacter.toLowerCase()))
		{
			boyfriend.y += (Math.sin(elapsedtime) * 0.6);
		}
		if(sideFloaters.contains(boyfriend.curCharacter.toLowerCase()))
		{
		...
		switch (curStage)
		{
			case 'farmNight' | 'houseNight':
				{
					dad.color = 0xFF878787;
					gf.color = 0xFF878787;
					boyfriend.color = 0xFF878787;
				}
			case 'farmSunset' | 'houseSunset':
				{
	...
	function ghostMiss(statement:Bool = false, direction:Int = 0) {
		if (statement) {
	...
	/*function windowDance(toggle:Bool) //ill just use lua this shit aint worth it-
		{
			windowDancing = toggle;

			var windowx:Int = Lib.application.window.x;
			var windowy:Int = Lib.application.window.y;

			if (windowDancing) {
				windowy = 1000*Math.sin(sineElap)/10+windowy;
				windowy = 1000*Math.cos(sineElap)/5+windowx;
			}
		}*/
	...
	override function beatHit()
	{
		...
		var thing:Float = 0.01;

		if (curBeat % 3 == 1){
			iconP1.setGraphicSize(Std.int(iconP1.width + (50 * (2 - thing))),Std.int(iconP1.height - (25 * (2 - thing))));
			iconP2.setGraphicSize(Std.int(iconP2.width + (50 * (2 - thing))),Std.int(iconP2.height - (25 * (2 - thing))));
		} else if (curBeat % 3 == 2){
			if (healthBar.percent > 20){
			...
			if (healthBar.percent > 20){
				FlxTween.angle(iconP1, 90, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
			} else {
				iconP1.setGraphicSize(Std.int(iconP1.width + (50 * (2 - thing))),Std.int(iconP1.height - (25 * (2 - thing))));
			}
			if (healthBar.percent < 80){
				FlxTween.angle(iconP2, -90, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
			} else {
				iconP2.setGraphicSize(Std.int(iconP2.width + (50 * (2 - thing))),Std.int(iconP2.height - (25 * (2 - thing))));
			}
		}
